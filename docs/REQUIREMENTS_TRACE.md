...
| R31 | Evaluators can discover and exercise every endpoint and see every response | Swagger UI `/api/doc`; generated `docs/openapi.json`; `ProblemSchema`; `#[OA\...]` on controllers | `OpenApiCoverageTest`, `OpenApiSnapshotTest`, `HealthEndpointTest` (`assertResponseIsDocumented`) | 0.7, 1.3, 4.2 | in progress |
