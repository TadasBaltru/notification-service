...
| R04 | Failover: if one provider fails, use another | `FailoverDeliveryStrategy` | `FailoverDeliveryStrategyTest::test_it_fails_over_to_next_provider_on_transient_failure`; `DeliveryPipelineTest::test_it_fails_over_to_the_secondary_when_the_primary_is_transient` | 2.2, 3.3 | done |
...
| R06 | Notifications must not get lost; retry later when all providers fail | `DeliverNotificationHandler` maps `retryLater` to `RecoverableMessageHandlingException`; `config/packages/messenger.yaml` retry + `failed` transport; delivery stays `pending` on exhaustion; `make retry-failed` | `DeliveryPipelineTest::test_it_keeps_the_delivery_pending_when_every_provider_fails_transiently`; `DeliverNotificationHandlerTest`; manual `make failed` / `make retry-failed` | 3.1, 3.3, 3.4 | done |
| R07 | Provider times out but may have accepted the message | `UnknownProviderOutcome` -> attempt `unknown`, no same-run failover, retry later | `FailoverDeliveryStrategyTest::test_it_does_not_fail_over_when_outcome_is_unknown`; `DeliveryPipelineTest::test_it_retries_later_without_failover_when_the_primary_times_out` | 2.2, 3.3 | done |
...
| R14 | Configuration: enable / disable channels | `notifications.channels.*.enabled`; disabled -> delivery `skipped` in `SendNotificationHandler` and not dispatched | `ChannelConfigurationTest`; `SendNotificationHandlerTest`; `DeliveryPipelineTest::test_it_skips_a_disabled_channel_and_queues_nothing` | 2.1, 3.1, 3.3 | done |
...
| R17 | Send one notification through several channels at once | `channels: [...]` in request -> one delivery per channel | `NotificationApiTest`; `DeliveryPipelineTest::test_it_delivers_every_requested_channel` | 1.3, 3.3 | done |
