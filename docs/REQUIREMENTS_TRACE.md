...
| R29 | Real provider where practical, no accounts required, no secrets committed | `SmtpMailerProvider` via `MAILER_DSN` (`null://null` until Mailpit in 3.2); `TwilioSmsProvider` via `TWILIO_*` (harmless defaults in `.env`, real values only in `.env.local`) | `SmtpMailerProviderTest`; `TwilioSmsProviderTest` | 2.3, 2.4, 3.2 | in progress |
