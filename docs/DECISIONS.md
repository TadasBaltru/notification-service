...
- 4.2 — the user list filters `user_id` and a `created_at` range. `idx_notifications_user_id` is replaced by
  `idx_notifications_user_created (user_id, created_at)`; the leftmost column still serves equality on `user_id`.
  Unique `idempotency_key` and unique `(notification_id, channel)` stay. A user id with no rows is an empty
  list, because there is no user table (§4.1). Unparsable `since` is HTTP 400.
