...
- `tools/check-layers.php` enforces the dependency rule (Domain has no `Symfony\` / `Doctrine\` imports;
  Application has no Infrastructure / HTTP / Doctrine imports). `deptrac` evaluated and rejected as heavyweight
  for a single bounded context; it is the production choice.
- One explicit exception in Domain: `Doctrine\Common\Collections` (`Collection` / `ArrayCollection`). It is a
  standalone library without ORM dependency, and a mapped one-to-many (`Notification -> Delivery ->
  DeliveryAttempt`) can only be hydrated into a `Collection`-typed property (the ORM substitutes
  `PersistentCollection` behind the same interface). The alternative — three unrelated aggregates with the
  "one channel per notification" invariant living only in a DB unique index — was rejected because it moves a
  business rule out of the model. Whitelisted in the checker, nothing else from Doctrine is.
...
## 4. Data model (*planned*; one migration per phase that introduces the need)
...
- Mapping: XML under `Infrastructure/Persistence/Doctrine/Mapping`, replacing the starter's `App` attribute
  mapping in `doctrine.yaml`. Doctrine's driver chain returns the first driver whose prefix matches, so an
  `App` attribute driver listed before the `App\NotificationPublisher\Domain\Model` XML driver would shadow it.
