...
| `src/NotificationPublisher/Infrastructure/Provider/Email/SmtpMailerProvider.php` | Infrastructure | SMTP adapter; deterministic Message-ID from the delivery id | `name()`; `channel()`; `send(OutboundMessage): ProviderResult` | SmtpMailerProviderTest |
| `src/NotificationPublisher/Infrastructure/Provider/Email/SmtpFailureClassifier.php` | Infrastructure | Maps an SMTP debug log to transient, recipient permanent, or unknown | `classify(string $provider, TransportExceptionInterface): never` | SmtpFailureClassifierTest |
