...
## Phase 4.2 — Tracking polish
- Proposed and accepted: `GET /notifications/{id}` adds `userId`, `createdAt`, `subject`, `body`, and each delivery's `provider`, `sentAt`, and `attempts` (the same document `POST` already returns, so one schema). `GET /users/{userId}/notifications` returns `{notifications: [...]}` with id, user, derived status, createdAt, and subject, newest first. `since` is inclusive (`created_at >=`). Unparsable `since` throws `BadRequestHttpException` before the bus, and the existing listener turns that into a `Problem` 400. `idx_notifications_user_id` is replaced by `idx_notifications_user_created (user_id, created_at)` via `doctrine:migrations:diff` (`Version20260924132829`). Unique `idempotency_key` and unique `(notification_id, channel)` were already present and stay.
- Rejected: HTTP 404 for an unknown user. There is no user table (§4.1); a user id with no rows and an id the resolver does not know are the same query result, an empty list. Rejected a JSON array as the list body: `assertResponseIsDocumented()` checks object keys, so the list is wrapped in `notifications`.
- Verified / corrected: Doctrine ORM 3 deprecates string `'DESC'` on `orderBy()`; the direction is `\SortDirection::Descending` (PHP 8.6 enum, polyfilled here). `doctrine:schema:validate` on dev is mapping and database `[OK]`. `--env=test` still reports `DROP TABLE messenger_messages` because the test transport is in-memory and the schema listener does not expect that table; that drift predates this phase. `TrackingApiTest` OK (6 tests, 384 assertions). Gate: phpunit `OK (110 tests, 2561 assertions)`, php-cs-fixer `Found 0 of 136 files`, phpstan `[OK] No errors`, `check-docs: OK (96 classes mapped)`, `check-layers: OK`.

## Phase 4.3 — Static quality pass
- Proposed and accepted: delete `AttemptOutcome::allowsFailover()` (the strategy branches on the exception type, and nothing called the method) and the unused `updatedAt()` / delivery `createdAt()` getters. The timestamp columns stay. `DeliveryNotFound` is a 404 titled "Delivery not found", separate from `NotificationNotFound`. `phpstan.dist.neon` stays at level 8.
- Rejected: mapping `ApplicationException` (config and `DeliveryRequiresRetry`) to HTTP 422. Those failures belong to container build or the worker, not the caller's payload. Rejected a UserInterface exception base for the single `MissingHandledResult` (it stays a 500).
- Verified / corrected: Messenger mapping is unchanged and matches DECISIONS §3 — `succeeded` returns, `failed` throws `UnrecoverableMessageHandlingException`, `retryLater` throws `DeliveryRequiresRetry` (not `RecoverableExceptionInterface`), throttle returns after a `DelayStamp`. HTTP mapping: 404 for a missing notification or delivery, 422 for unknown user / missing contact / other `DomainException`, 400/404/422 for `HttpException`, 422 for validation. `JsonExceptionListenerTest` OK (3 tests, 14 assertions). Gate: phpunit `OK (111 tests, 2565 assertions)`, php-cs-fixer `Found 0 of 136 files`, phpstan `[OK] No errors`, `check-docs: OK (96 classes mapped)`, `check-layers: OK`.
